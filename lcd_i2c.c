...
void lcd_set_cursor(int line, int position) {
    ...
    if(line == 0) val = 0x00 + position;
    else if(line == 1) val = 0x40 + position;
    else if(line == 2) val = 0x00 + position + 0x14;
    else if(line == 3) val = 0x40 + position + 0x14;
...
void lcd_custom_char(uint8_t position, uint8_t data[]) {
    if(position >= 0 && position < 8){
        lcd_send_byte(LCD_SETCGRAMADDR | position * 8, LCD_COMMAND); // Set CGRAM address

        for (int i = 0; i < 8; i++) {
            lcd_send_byte(data[i], LCD_CHARACTER); // Enviar los datos al CGRAM
        }
    }
    else {
        return;
